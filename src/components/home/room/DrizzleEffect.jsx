...
const DrizzleEffect = ({weather}) => {
    const rainRef = useRef();
    const groundRef = useRef();
    const sunRef = useRef();
    const {scene: drizzleScene, clock, camera} = useThree();

    // clone sun model
    const {scene: originalSun} = useGLTF("/models/sun.glb");
    const sun2 = useMemo(() => originalSun.clone(true), [originalSun]);
    ...
    for (let i = 0; i < rainCount; i++) {
        ...
        positions[i * 6 + 4] = y - 2;
        positions[i * 6 + 5] = z;
        speeds.push(0.1 + Math.random() * 0.1);
    }
    ...
    return (
        <>
            ...
            {/* transparent sun */}
            <primitive ref={sunRef} object={sun2} position={[-8, 75, 15]} scale={7}/>
        </>
    );
};
